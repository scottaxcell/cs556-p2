...
my $barabasiLog = "barabasiCureRuns.log";
...
foreach my $barabasiFile (@barabasiFiles) {
  ...
  for (1..10) { # Run the experiment 10 times
    my $cmd = "python ./simulateWormCure.py --infect_prob $infectProb --infect_zero $infectZero --cure_prob $cureProb --cure_zero $cureZero --csv $barabasiFile";
    my $output = `$cmd`;
